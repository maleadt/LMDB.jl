...
using Test
    ...
    @test val == LMDB.mdb_unpack(String, mdb_val_ref)
    ...
    @test val == value
    @test val == LMDB.mdb_unpack(Vector{Int}, mdb_val_ref)
    ...
    value = unsafe_wrap(Array, convert(Ptr{T}, mdb_val.mv_data), nvals)
    @test val == value
    @test val == LMDB.mdb_unpack(Vector{UInt16}, mdb_val_ref)
    ...
    value = unsafe_wrap(Array, convert(Ptr{T}, mdb_val.mv_data), nvals)
    @test val == value
    @test val == LMDB.mdb_unpack(Vector{T}, mdb_val_ref)
    @test val[1] == LMDB.mdb_unpack(T, mdb_val_ref)
